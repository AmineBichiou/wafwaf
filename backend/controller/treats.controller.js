...
exports.createTreat = async (req, res, next) => {
    upload(req, res, function(err) {
        if (err) {
            ...
            try{
                const newTreat = treatServices.addTreat(treatName, treatDescription, treatImage);
                res.json(newTreat);
            }
            catch (error) {
                return next(error);
        ...
        upload(req, res, async function(err) {
            if (err) {
                ...
                try {
                    ...
                    const { id } = req.params;
                    const { treatName, treatDescription } = req.body;
                    const treatImage = path !== "" ? "/" + path : "";

                    const updatedTreat = await treatServices.updateTreats(id, treatName, treatDescription, treatImage);
